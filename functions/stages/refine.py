...
class RefinementConfig:
    ...
    params: dict = field(default_factory=lambda: {
        ...
        # 2) reconstruction
        # Reduced depth for smoother results and better robustness on complex/noisy shapes
        "generate_surface_reconstruction_screened_poisson": {
            "depth": 10,          # Reduced from 10 to 8 for smoothing
            "fulldepth": 8,      # Reduced from 8 to 6
            # 'cgdepth': 0,
            # 'scale': 1.1,
            # 'samplespernode': 1.5,
            # 'pointweight': 4,
            # 'iters': 8,
            # 'confidence': False,
            # "preclean": True,    # Enabled preclean to help with noise
            # 'threads': 16,
        },
...
def stage_refine(cfgs, debug_mode: bool = False):
    ...
    #     )
    # except Exception as e:
    #     print(f"[warning] Poisson reconstruction failed with default settings: {e}")
    #     print("[info] Retrying with even lower depth (smoother)...")
    #     # Fallback to even smoother settings if it fails
    #     fallback_params = refCfgs.params["generate_surface_reconstruction_screened_poisson"].copy()
    #     fallback_params["depth"] = 6
    #     fallback_params["fulldepth"] = 5
    #     ms.generate_surface_reconstruction_screened_poisson(**fallback_params)
    #
